...
function removeLastOccurrences(str, value) {
  ...
  const left = str.slice(0, ind);
  const right = str.slice(ind).replace(value, '');
  return left + right;
}
...
function sumOfCodes(str) {
  if (!str) return 0;
  let result = 0;
  for (let i = 0; i < str.length; i += 1) result += str[i].charCodeAt();
  return result;
}
...
function extractNameFromTemplate(value) {
  return value.replaceAll(/!|,/g, '').split(' ').slice(1).join(' ');
}
...
function encodeToRot13(str) {
  ...
  const result = str.split('').map((elem) => {
    return input.indexOf(elem) < 0 ? elem : output[input.indexOf(elem)];
  });
  return result.join('');
}
